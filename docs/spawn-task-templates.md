...
无论使用长模板还是短模板,任务相关回复都必须包含:

1. `当前任务:<任务 ID 或任务名> / <状态>`
2. `本次推进:<本条回复完成了什么>`
3. 如果当前任务已完成,再补充:
   - `本进程其他进行中:<列表;无则写 无>`
   - `本进程其他未完成:<列表;无则写 无>`
...
完成时:
- 清晰总结结果。
- 包含:`当前任务:<任务 ID 或任务名> / Done`
- 包含:`本次推进:<完成了什么>`
- 包含:`本进程其他进行中:<列表;无则写 无>`
- 包含:`本进程其他未完成:<列表;无则写 无>`
- 包含:调度链:小青 -> <role> -> 结果汇总
```
...
完成时包含:当前任务、本次推进、本进程其他进行中、本进程其他未完成、调度链:小青 -> plan -> 结果汇总
```
...
完成时包含:当前任务、本次推进、本进程其他进行中、本进程其他未完成、调度链:小青 -> architect -> 结果汇总
```
...
产出:给出代码改动、改动说明和验证结果。
完成时包含:当前任务、本次推进、本进程其他进行中、本进程其他未完成、调度链:小青 -> coder -> 结果汇总
```
...
完成时包含:当前任务、本次推进、本进程其他进行中、本进程其他未完成、调度链:小青 -> env-engineer -> 结果汇总
```
...
完成时包含:当前任务、本次推进、本进程其他进行中、本进程其他未完成、调度链:小青 -> debugger -> 结果汇总
```
...
完成时包含:当前任务、本次推进、本进程其他进行中、本进程其他未完成、调度链:小青 -> tester -> 结果汇总
```
...
**label**
```text
toc-monitor - <check_type>
```

**task**
```text
你是 toc-monitor 角色。
在做任何实质性动作之前，先阅读 memory/long-term/BEHAVIOR-PROTOCOL-TOC-MONITOR.md 并遵循其中规则。

任务：
执行 TOC 系统状态检查并生成报告

检查类型：<轻量检查/完整检查/实验评估/周复盘准备>
重点关注：<如有特定关注点>

上下文 / 输入：
- 读取 tasks/ 目录下的所有 TaskCard
- 读取 state/metrics-snapshots/ 下的历史快照
- 读取 state/constraint-record.yaml 当前制约记录
- 读取 logs/ 下的系统日志
- 使用 scripts/toc-state-check.py 和 scripts/toc-algorithm-verification.py 作为工具

约束：
- 聚焦于数据分析和制约识别，不干预任务执行
- 保持检查高效：轻量检查<2分钟，完整检查<5分钟
- 如发现问题，通过小青协调解决，不直接行动
- 输出必须结构化，便于小青快速理解
...
完成时：
- 通过 subagent_announce 通知小青
- 包含：调度链：小青 -> toc-monitor -> 结果汇总
```

...
- 当前说明：
  - 当前可用主路径为两条：WSL 自管浏览器；Windows 真实浏览器的 `chrome-relay` remote CDP 链路
  - 当前已验证 `chrome-relay` 可用端点：`http://172.31.208.1:9223`
  - 旧的 `9222` / `18792` 尝试链路不作为当前默认维护路径
- 下一步：
  - 浏览器公开页 / 无登录页任务优先分流给青览处理
  - 需要真实 Windows 浏览器态时优先走 `chrome-relay`
  - 需要登录态、2FA、验证码或高风险写操作时，再切人工浏览器接管
  - 后续择机清理这次排障留下的临时脚本与旧转发痕迹
...
- 已完成：
  ...
  - 已接入每日首次汇报规则
  - 已补充固定模板、固定来源池与“多源防茧房”规则
  - 已明确新闻条目必须标注日期与来源
  - 已明确数量规则为“每栏最多 10 条”，不是总共 10 条
  - 已跑出一版正式模板候选
- 当前暂停点：
  - 当前版本模板已基本成型，但外部主流报道层补强仍不足
  - AI / 科学 / 中国三栏的来源平衡度还需要继续提升
- 下一步（恢复时）：
  - 优先补 Reuters / FT / Bloomberg / AP / Nature News / 财新 / 澎湃 等外部主流源
  - 在不牺牲真实性的前提下，继续优化成正式终版

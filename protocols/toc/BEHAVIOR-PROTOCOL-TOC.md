...
## 7. 关键对象
- **TaskCard**：系统中的唯一任务对象（模板：`templates/data/task-card-template.yaml`）
- **ConstraintRecord**：当前主制约记录（`state/constraint-record.yaml`）
- **DecisionPacket**：需要俊阳裁决的事项包（模板：`templates/data/decision-packet-template.yaml`）
- **ExperimentCard**：提升制约的实验记录（模板：`templates/data/experiment-card-template.yaml`）

## 8. 监控角色
- **toc-monitor**：TOC 系统状态监控器，负责数据分析、制约识别、报告生成
  - 协议文件：`protocols/toc/BEHAVIOR-PROTOCOL-TOC-MONITOR.md`
